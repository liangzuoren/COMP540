...
class LinearRegressor:
    ...
    def predict(self, X):
        ...
        #    One line of code expected                                            #
        ###########################################################################
        #predicted values h_theta(x) = (theta^T)x
...
class LinearReg_SquaredLoss(LinearRegressor):
    ...
    def loss (self,X,y):
        ...
        # errors = predicted values - y
        errs = X.dot(self.theta) - y
        # loss function
        J = (1.0 / (2 * num_examples)) * (errs ** 2).sum()
        # gradient
        grad = np.array([(1.0 / num_examples) * np.multiply(errs,X[:,j]).sum() for j in range(dim)])
