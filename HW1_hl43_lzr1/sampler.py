...
class ProbabilityModel:
    ...
    def sample_many(self, n):
        # draw n independ samples from a given distribution
...
class UnivariateNormal(ProbabilityModel):
    ...
    def __init__(self,mu,sigma):
        ...
        self.sigma = sigma
    def sample(self):
        # Use Box Muller method to generate values from normal distribution
        # Generate two independent random numbers distributed uniformly on 0 to 1
        u = np.random.uniform(0,1,2)
        # x is a random sample from standard normal distribution (mu =0, sigma = 1)
...
class MultiVariateNormal(ProbabilityModel):
    ...
    def sample(self):
        ...
        # initialize a univariate normal distribution model with random mu and sigma (will update them later)
        UniNorm = UnivariateNormal(0,1)
        # draw D independent standard normal variates using their respective means and variances
        z = UniNorm.sample_many(2)
        x = self.Mu + A.dot(z)
        return x
...
class Categorical(ProbabilityModel):
    ...
    def __init__(self,ap):
        self.ap = ap
    def sample(self):
        # generate an independent random numbers distributed uniformly on (0,1)
        u = np.random.uniform(0,1)
        # compute the cumulative probability of each categoty
        cumProb = self.ap.cumsum()
...
class MixtureModel(ProbabilityModel):
    ...
    def __init__(self,ap,pm):
        ...
        self.pm = pm
    def sample(self):
        CateDist = Categorical(self.ap)
        dist_model = self.pm[CateDist.sample()]
        return dist_model.sample()
